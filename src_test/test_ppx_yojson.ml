...
type pv_native = [ `A | `B of int | `C of int * string ]
[@@deriving show, yojson { variants = `Native }]
...
[@@deriving show, yojson { variants = `Adjacent ("tag", "contents") }]
type rv_native = RA | RB of int | RC of int * string | RD of { z : string }
[@@deriving show, yojson { variants = `Native }]
...
let test_pvar_native _ctxt =
  assert_roundtrip pp_pv_native pv_native_to_yojson pv_native_of_yojson
                   `A "<\"A\">";
  assert_roundtrip pp_pv_native pv_native_to_yojson pv_native_of_yojson
                   (`B 42) "<\"B\":42>";
  assert_roundtrip pp_pv_native pv_native_to_yojson pv_native_of_yojson
...
let test_recvar_native _ctxt =
  assert_roundtrip pp_rv_native rv_native_to_yojson rv_native_of_yojson
                   RA "<\"RA\">";
  assert_roundtrip pp_rv_native rv_native_to_yojson rv_native_of_yojson
                   (RB 42) "<\"RB\":42>";
  assert_roundtrip pp_rv_native rv_native_to_yojson rv_native_of_yojson
...
let suite = "Test ppx_yojson" >::: [
    ...
    "test_pvar_native" >:: test_pvar_native;
    "test_var"       >:: test_var;
    "test_rec"       >:: test_rec;
    ...
    "test_recvar_native" >:: test_recvar_native;
    "test_key"       >:: test_key;
    "test_id"        >:: test_id;
